...
# Mets ton image dans assets/menu_bg.jpg (ou change le chemin ici)
BG_IMAGE_ORIG = None
try:
...
def police(size: int) -> pygame.font.Font:
    """Retourne une police Pygame mise en cache."""
    if size in _FONT_CACHE:
        return _FONT_CACHE[size]
    f = pygame.font.Font(None, size)   # ou SysFont("arial", size) si tu préfères
    _FONT_CACHE[size] = f
    return f

def _wrap_text(text: str, font: pygame.font.Font, max_width: int):
    """Coupe un texte en plusieurs lignes pour tenir dans max_width pixels."""
    words = text.split()
    lines = []
    current = ""
...
def draw_background(surface):
    """Dessine l'image de fond (ou un fond uni si pas d'image)."""
...
def main_menu(screen, size, title_font, btn_font, foot_font, partie_en_cours: bool):
    ...
    - Jouer / Reprendre la partie  (ouvre un sous-menu)
    - Découverte
    - Options
    - Quitter
            ...
            for b in buttons:
                if b.is_clicked(event):
                    if b.text.startswith("Jouer"):
                        # Ouvre le sous-menu "Reprendre / Nouvelle partie"
...
def play_or_resume_menu(screen, size, btn_font, foot_font, partie_en_cours: bool):
    """
    Affiche un petit menu au milieu de l'écran avec :
    - Reprendre la partie (désactivé si aucune partie en cours)
    - Nouvelle partie
    - Retour

    Retourne :
    - "RESUME"   si on choisit Reprendre
    - "NEW_GAME" si on choisit Nouvelle partie
    - "BACK"     si on annule
    """
    WIDTH, HEIGHT = size
    ...
    while True:
        for event in pygame.event.get():
            ...
            # Reprendre la partie (seulement si partie en cours)
        ...
        # titre
        title_font = pygame.font.Font(None, max(40, HEIGHT // 16))
        draw_text_center(screen, "Jouer / Reprendre", title_font, WHITE, (WIDTH // 2, box.y + 55))

        # boutons
        # Reprendre grisé si aucune partie
        if not partie_en_cours:
            # on dessine le bouton normalement, mais on ajoute un voile gris
...
class RoomCatalogueScreen:
    """
    Catalogue des pièces (Découverte -> Catalogue des pièces).

    - Z / S : changer d'étage (couleur de pièces).
    - Q / D : changer de pièce dans l'étage courant.
    - Échap / Retour : revenir au menu Découverte.
    """
    ...
    def __init__(self, screen, size, title_font, btn_font, foot_font):
        ...
        # Couleurs d'encadrement (les vraies couleurs en jeu, importées depuis jeu.py)
        self.couleur_rgb = {
            "Bleue":    C_BLEU,
            "Verte":    C_VERT,
            "Violette": C_VIOLET,
        ...
        # On regroupe les rooms par couleur et on charge leurs images
        ...
        # On trie chaque liste par ordre alphabétique
    ...
    def draw(self):
        ...
        # Texte explicatif juste en dessous
        ...
        # Description en bas de la carte
        ...
        desc = room.get("desc", "")
        if desc:
            # on coupe en 2 lignes max (simple)
            mots = desc.split()
            line1 = []
            line2 = []
            for m in mots:
...
class DiscoveryMenu:
    """Sous-menu 'Découverte' avec navigation vers 4 sections, dont 'Objets' (sous-sous-menu)."""
    def __init__(self, screen, size, title_font, btn_font, foot_font):
        self.screen = screen
        self.size = size
        self.title_font = title_font
        ...
        self.buttons = [
            Button("Catalogue des pièces",         (self.WIDTH//2, self.start_y),              width=max(380, self.WIDTH//3), height=max(70, self.HEIGHT//12), font=self.btn_font),
            Button("Inventaire",                   (self.WIDTH//2, self.start_y + self.gap),   width=max(380, self.WIDTH//3), height=max(70, self.HEIGHT//12), font=self.btn_font),
            Button("Objets",                       (self.WIDTH//2, self.start_y + 2*self.gap), width=max(380, self.WIDTH//3), height=max(70, self.HEIGHT//12), font=self.btn_font),
        ]
...
class ScrollListScreen:
    """Liste scrollable d'items avec icône (optionnelle) + titre + description."""
    ...
    def run(self):
        ...
        while True:
            ...
            self.back_btn.draw(self.screen)
            draw_footer(self.screen, (self.W//2, self.H), self.foot_font, text="Molette/↑↓ pour défiler — Échap ou Retour")
...
class OptionsScreen:
    ...
    def run(self):
        ...
        while running:
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    # On ne quitte pas ici : on laisse main() gérer
                    running = False
                    ...
                    # Valider (Entrée / Espace)
                        ...
                        # 1) Volume : rien à faire, il est déjà appliqué
                        if current == "Volume musique":
                            pass  # on reste sur l'écran

                        # 2) Changer résolution : on ouvre le petit écran de choix
...
def run_blue_prince_loop(w, h, fullscreen, scene_jeu=None):
    """
    Lance la boucle du vrai jeu (jeu.SceneJeu) avec la MÊME résolution que le menu.
    - w, h : résolution choisie dans le menu (ou taille écran si plein écran)
    - fullscreen : True si tu as choisi plein écran dans le menu
    - scene_jeu : None = nouvelle partie, sinon on reprend cette instance
    """

    # même mode que le menu
    if fullscreen:
    ...
    if scene_jeu is None:
        scene_jeu = SceneJeu()   # <-- ta classe de jeu dans jeu.py
